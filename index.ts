...
export function when<T>(condition: (val: T, ev: ReactiveEvent<T>) => boolean, callback: ReactiveUpdater<T>, ...reactives: Reactive<T>[]) :Unsubscriber {
    return onChange((value, ev) => condition(value, ev) && callback(value, ev), true, ...reactives);
}
export function update<T>(re: Reactive<T>, callback: (val: T, ...args: any[]) => T, ...args: any[]) {
    re.value = callback(re.value as T, ...args);
}

export async function asyncWhen<T>(condition: (val :T, ev: ReactiveEvent<T>) => Promise<boolean>, callback: (currentValue: T, ev: ReactiveEvent<T>) => void, ...reactives: Reactive<T>[]) :Promise<Unsubscriber> {
    return onChange(async (value, ev) => {
